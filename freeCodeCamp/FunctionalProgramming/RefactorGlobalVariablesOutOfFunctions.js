...
function add (arr, bookName) {
    let workBookList = arr.slice(0);
    workBookList.push(bookName);
    return workBookList;
}
...
function remove (arr, bookName) {
     let workBookList = arr.slice(0);
     let x = workBookList.indexOf(bookName);
     if (x >= 0) {
         workBookList.splice(x, 1);
         return workBookList;
     }     
     return arr;
...
The add function should add the given bookName to the end of an array. The remove function should 
remove the given bookName from an array. Both functions should return an array, 
