...
filteredList = watchList.filter(item => item.imdbRating >= 8).map((item) => { return {title: item.Title, rating: item.imdbRating} });;
...
but where imdbRating is greater than or equal to 8.0. Note that the rating values are saved as strings in 
