...
function longRepeat(str) {
    ...
    //create new array from string and convert characters to lowercase and filer out everything except letters.
    let arr = str.toLowerCase().split("").filter(element => element >= "a" && element <= "z");     
    ...
    arr.forEach(function(element) {
        if (element != prevElement) {
            prevElement = element;
            if (prevString.length > maxStringLen) {
                maxStringLen = prevString.length;
...
//let maxLen = longRepeat('sdsffffse'); // == 4
let maxLen = longRepeat('ddvvrwwwrggg'); // == 3

//let maxLen = longRepeat('aaaxxxxxxxxxxxxxxxxa'); // == 3
